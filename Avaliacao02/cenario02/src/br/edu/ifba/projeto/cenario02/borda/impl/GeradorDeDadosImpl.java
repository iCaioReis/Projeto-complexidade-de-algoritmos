...
public class GeradorDeDadosImpl implements Runnable {
    ...
    protected Map<Veiculo, Float> veiculosEficiencia = new TreeMap<>();
    ...
    // Complexidade constante O(1)
    public Map<Veiculo, Float> getVeiculosComEficiencia() {
        return veiculosEficiencia;
    }
    // Complexidade  linear, O(N^2), pois existe um for que chama o método medirEficienciaVeiculo de complexidade o(N)
    public Map<Veiculo, List<LeituraVeiculo>> gerarVeiculos(String filial, int TOTAL_DE_VEICULOS, int TOTAL_DE_LEITURAS) throws InterruptedException {
        ...
        for (Map.Entry<Veiculo, Sensor> entry : veiculoSensor.entrySet()) {
            ...
            medirEficienciaImpl eficiencia = new medirEficienciaImpl();
            float eficienciaVeiculo = eficiencia.medirEficienciaVeiculo(sensor.getLeituras());

            veiculos.put(veiculo, sensor.getLeituras());
            veiculosEficiencia.put(veiculo, eficienciaVeiculo);
        }
        return veiculos;
    }
